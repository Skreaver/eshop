...
public class SORM {
	...
	private void classTest(Class<?> clazz) throws SORMException{
		if(clazz.getAnnotation(Table.class) == null)
			throw new SORMException("Class " + clazz.getName() + " doesnt include annotation @Table");
		...
		for (Field field : clazz.getDeclaredFields()){
			...
			if(column != null && column.primaryKey()){
				if(!field.getType().getSimpleName().equals("int")){
					throw new SORMException("Primary key must be int type.");
				}
				idCount++;
			}
		}
		if(idCount != 1)
			throw new SORMException("Class with annotation @Table must include only one primary key");
	}
